    ...
    <main className="site">
      ...
      {view==='world'&&<>
        ...
        {scene==='street'?<section className="world-scene">
          <div className="sky"><div className="sun"></div><div className="building-title">SPENCER PLAZA</div><div className="building-sub">SHOPPING · FOOD · MUSIC · INTERNET</div></div>
          <div className="plaza-building"><div className="roofline"></div><div className="arches"><span>01</span><span>02</span><span>03</span><span>04</span><span>05</span></div><div className="shop-row"><button onClick={()=>setShopOpen(true)}>PLANET M</button><button onClick={()=>setShopOpen(true)}>DENIM</button><button onClick={()=>setShopOpen(true)}>CD WORLD</button><button onClick={()=>setShopOpen(true)}>BOOKS</button><button onClick={()=>setShopOpen(true)}>PHOTO</button></div></div>
          <div className="street-lane"><div className={`arrival-vehicle ${playerGender==='girl'?'girl-arrival':''}`} style={{left:`${Math.min(58,8+arrival*2.2)}%`}}>{playerGender==='girl'?<><span className="maruti">MARUTI 800</span><span className="maruti-person m1"></span><span className="maruti-person m2"></span></>:<><span className="bike-wheel w1"></span><span className="bike-wheel w2"></span><span className="bike-body">RX 100</span><span className="bike-rider"></span></>}</div><div className="arrival-caption">{arrival<6?'A Yamaha RX 100 pulls into the plaza…':'You have arrived.'}</div></div>
          <div className="plaza-floor">{NPCS.map(n=><div key={n.id} className="npc" style={{left:`${n.x}%`,top:`${n.y}%`,'--dir':n.dir}} title={`${n.name} · ${n.role}`}><span className="npc-head"></span><span className="npc-body"></span><span className="npc-label">{n.name}</span></div>)}<div className="player" style={{left:`${player.x}%`,top:`${player.y}%`}}><span className="player-head"></span><span className="player-body"></span><b>YOU</b></div><button className={`flyer ${flyerVisible?'revealed':''}`} onClick={enterCafe}><strong>LOOKING FOR SOMEONE?</strong><span>Maybe they're online.</span><small>NET CAFÉ · 1st FLOOR →</small></button><button className="cafe-door" onClick={enterCafe}>NET CAFÉ<br/><small>1ST FLOOR</small></button><div className="plant p1"></div><div className="plant p2"></div><div className="bench">BENCH</div></div>
          <div className="world-controls"><button onClick={()=>movePlayer(-3,0)}>←</button><button onClick={()=>movePlayer(0,-3)}>↑</button><button onClick={()=>movePlayer(0,3)}>↓</button><button onClick={()=>movePlayer(3,0)}>→</button><button onClick={()=>setNotice('Explore the plaza. After 120 seconds the flyer becomes clear and the Net Café opens.')}>?</button></div>
          <div className="world-hint">{flyerVisible?'THE FLYER CAUGHT YOUR EYE · ENTER THE NET CAFÉ':`EXPLORE SPENCER PLAZA · FLYER IN ${Math.max(0,120-arrival)}s`}</div>
        </section>:<Cafe computer={computer} setComputer={setComputer} leaveCafe={leaveCafe} openMail={()=>{setView('mail');setSelected(null);}} openShops={()=>setShopOpen(true)} />}
        <div className="world-shelf"><b>MORE WORLDS</b><span>MARINA BEACH · FREE SOON</span><span>🔒 KASHMIR · 2004</span><span>🔒 ROOFTOP DATE · 2005</span><span>🔒 TOKYO · 2006</span></div>
      </>}
